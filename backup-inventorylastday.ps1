...
$outputfile = "Backupinventorylastday_"+$date+".csv"
$beginDay = (get-date -Hour 0 -Minute 00 -Second 00).ToUniversalTime()
$starttime = $beginDay.AddDays(-2).ToUniversalTime()
$endtime = $beginDay.AddDays(-1).AddSeconds(-1).ToUniversalTime()
$arr = @()
    ...
    if($sub.name -ne "ABI AFRICA PROD" -and $sub.name -ne "ABI AFRICA NON-PROD")
    {
        try 
	    {
            ...
            {        
                ...
                $vmlist = Get-AzRecoveryServicesBackupJob -From $starttime -To $endtime
